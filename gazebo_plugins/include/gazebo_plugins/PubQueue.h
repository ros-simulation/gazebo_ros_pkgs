...
public:
  ...
  rclcpp::Publisher<T> pub_;
  PubMessagePair(T &msg, rclcpp::Publisher<T> &pub) : msg_(msg), pub_(pub) {}
};
...
public:
  PubQueue(QueuePtr queue,
           ...
           boost::function<void()> notify_func) : queue_(queue), queue_lock_(queue_lock), notify_func_(notify_func) {}
  ~PubQueue() {}
  ...
  void push(T &msg, rclcpp::Publisher<T> &pub)
  {
...
public:
  ...
  void spin()
  {
    while (rclcpp::ok() && service_thread_running_)
    {
