...
: d_ptr_(new GazeboRosTemplatePrivate)
{
}
...
void GazeboRosTemplate::Load(physics::ModelPtr model, sdf::ElementPtr sdf)
{
  // Create a GazeboRos node instead of a common ROS node.
  // Pass it SDF parameters so common options like namespace and remapping
  // can be handled.
  d_ptr_->ros_node_ = gazebo_ros::Node::Create("gazebo_ros_template", sdf);

  // The model pointer gives you direct access to the physics object,
  // for example:
  RCLCPP_INFO(d_ptr_->ros_node_->get_logger(), model->GetName().c_str());

  // Create a connection so the OnUpdate function is called at every simulation
  // iteration. Remove this call, the connection and the callback if not needed.
  d_ptr_->update_connection_ = event::Events::ConnectWorldUpdateBegin(
