...
rosrun gazebo_ros spawn_model -file `echo $GAZEBO_MODEL_PATH`/coke_can/model.sdf -sdf -model coke_can1 -y 0 -x 1

read -p "SDF from local model database with Deprecated tag:"
rosrun gazebo_ros spawn_model -file `echo $GAZEBO_MODEL_PATH`/coke_can/model.sdf -gazebo -model coke_can2 -y 0 -x 2

read -p "SDF from the online model database:"
rosrun gazebo_ros spawn_model -database coke_can -sdf -model coke_can3 -y 0 -x 3
