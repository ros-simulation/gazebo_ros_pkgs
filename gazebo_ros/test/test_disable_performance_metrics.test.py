...
class TestPerformanceMetricsParam_disable(unittest.TestCase):
    ...
    def test_parameter_disable(self):
        """ Checks if /performance_metrics topic stops publishing if enable_performance_metrics is set to False  """
        node = MakeTestNode('performance_metrics_disable_testing_node')
        assert node.wait_for_gazebo_node(timeout_sec = 5.0), 'Gazebo node not found, parameter could not be set'
        ...
        time.sleep(2.5)
        assert node.msg_count == 0, f'Received {node.msg_count} messages after setting enable_performance_metrics parameter to False, test failed'
...
class TestPerformanceMetricsParam_enable(unittest.TestCase):
    ...
    def test_parameter_enable(self):
        """ Checks if /performance_metrics topic starts publishing if enable_performance_metrics is set to True  """
        ...
        response = node.set_parameter(True)
        assert response.successful, 'Parameter could not be set to True'

        node.start_subscriber()
        time.sleep(2.5)
        assert node.msg_count != 0, 'Did not receive any messages after setting enable_performance_metrics to True, test failed'
...
class MakeTestNode(Node):
    ...
    def start_subscriber(self):
        """ Start listening for messages on /performance_metrics topic, run rclpy.spin() on a separate thread  """
